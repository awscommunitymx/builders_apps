...
interface TvDisplayStackProps extends cdk.StackProps {
  apiUrl: string;
  apiKey: string;
  environment: string;
...
export class TvDisplayStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: TvDisplayStackProps) {
    ...
    // Create S3 bucket to host the TV display app
    const websiteBucket = new s3.Bucket(this, 'TvDisplayBucket', {
    ...
    const distribution = new cloudfront.Distribution(this, 'TvDisplayDistribution', {
      defaultBehavior: {
    ...
    new route53.AaaaRecord(this, 'TvDisplayAliasAaaa', {
      zone: hostedZone,
      target: route53.RecordTarget.fromAlias(new targets.CloudFrontTarget(distribution)),
      recordName: props.domainName,
    });

    new route53.ARecord(this, 'TvDisplayAliasA', {
      zone: hostedZone,
      target: route53.RecordTarget.fromAlias(new targets.CloudFrontTarget(distribution)),
      recordName: props.domainName,
    });
    ...
    new s3deploy.BucketDeployment(this, 'DeployTvDisplayWebsite', {
      sources: [s3deploy.Source.asset(path.join(tvDisplayPath, 'dist'))],
    ...
    new cdk.CfnOutput(this, 'TvDisplayWebsiteURL', {
      value: `https://${props.domainName}`,
      description: 'TV Display Website URL',
    });

    // Output the distribution ID for future reference
    new cdk.CfnOutput(this, 'TvDisplayDistributionId', {
      value: distribution.distributionId,
      description: 'TV Display CloudFront Distribution ID',
    });
  }
}